...
def check_translations():
    ...
    with open('saver.html', 'r', encoding='utf-8') as f:
        html = f.read()

    keys_in_html = set(re.findall(r'data-translate="([^"]+)"', html))
    print(f"Total unique translation keys in saver.html: {len(keys_in_html)}")
...
def check_structure():
    ...
    why_slides_img = re.findall(r'<div class="swiper-slide">\s*<div class="image-wrapper">\s*<img src="\./The Saver - Golfyr_files/[^"]+"', html, re.DOTALL)
    ...
    if len(why_tabs) != 4 or len(why_slides_img) != 4 or len(why_slides_desc) != 4:
        print("Error: Why choose tabs/slides count is not exactly 4!")
        return False
    
    if len(spec_slides) != 3:
        print("Error: Tech specs slides count is not exactly 3!")
        return False
...
def check_links():
    ...
    for filepath in all_html_files:
        ...
        saver_links = re.findall(r'href=["\']\./index\.html#saver["\']|href=["\']#saver["\']', content)
        if saver_links:
            print(f"Error: Found lingering saver section links in {filepath}: {saver_links}")
            failures = True

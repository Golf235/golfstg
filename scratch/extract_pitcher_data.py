...
class PitcherDetailExtractor(HTMLParser):
    def __init__(self):
        super().__init__()
...
def run_extraction(url, lang):
    ...
    try:
        ...
        print("\nWhy Slides Found:")
        # We need to filter why slides to match only those that have a title or desc
        ...
        for img in sorted(parser.images):
            if 'pitcher' in img.lower() or 'why' in img.lower() or 'spec' in img.lower():
                print(f"  {img}")
        
    except Exception as e:
        print(f"Error: {e}")

run_extraction("https://golfyr.com/sevenclubgame/the-pitcher/?v=d88fc6edf21e", "ENGLISH")
run_extraction("https://golfyr.com/de/sevenclubgame/the-pitcher/", "GERMAN")

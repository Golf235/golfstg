...
class SaverDetailExtractor(HTMLParser):
    def __init__(self):
        super().__init__()
...
def run_local_extraction(filepath, lang):
    ...
    for img in sorted(parser.images):
        if 'saver' in img.lower() or 'why' in img.lower() or 'spec' in img.lower() or 'header' in img.lower():
            print(f"  {img}")

run_local_extraction("scratch/saver_en.html", "ENGLISH")
run_local_extraction("scratch/saver_de.html", "GERMAN")
